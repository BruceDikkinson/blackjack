...
@property (strong, nonatomic) void (^callBackBlock)(TAlertView *alertView);
...
-(id)init
{
    return [self initWithTitle:nil message:nil buttons:nil andCallBack:^(TAlertView *alertView){}];
}

-(id)initWithTitle:(NSString*)title andMessage:(NSString*)message
{
    return [self initWithTitle:title message:message buttons:nil andCallBack:^(TAlertView *alertView){}];
}

-(id)initWithTitle:(NSString*)title message:(NSString*)message buttons:(NSArray*)buttons andCallBack:(void (^)(TAlertView *alertView))callBackBlock
{
    if (self = [super init]) {
...
-(void)show
{
    ...
    [groundCollisionBehavior addBoundaryWithIdentifier:@"ground"
                                             fromPoint:CGPointMake(CGRectGetMinX(self.frame), CGRectGetMidY(self.frame)+175.0f)
                                               toPoint:CGPointMake(CGRectGetMaxX(self.frame), CGRectGetMidY(self.frame)+175.0f)];
    ...
    CGFloat angularVelocity = (((float)rand() / RAND_MAX)-0.5f)*0.0f;
...
-(void)addSubviewsWithOverlay:(BOOL)withOverlay
{
    ...
    CGFloat outsideMargin = self.frame.size.width/5.0f; //15
